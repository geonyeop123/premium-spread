...
object DefaultJobConfigProvider : JobConfigProvider {
    override fun get(jobId: JobId): JobConfig = when (jobId) {
        ...
        JobId.PREMIUM_AGGREGATION_MINUTE -> JobConfig(jobId, "lock:aggregation:minute", Duration.ofSeconds(30), Duration.ofSeconds(20))
        JobId.PREMIUM_AGGREGATION_HOUR -> JobConfig(jobId, "lock:aggregation:hour", Duration.ofSeconds(60), Duration.ofSeconds(45))
        JobId.PREMIUM_AGGREGATION_DAY -> JobConfig(jobId, "lock:aggregation:day", Duration.ofSeconds(120), Duration.ofSeconds(90))
        JobId.TICKER_AGGREGATION_MINUTE -> JobConfig(jobId, "lock:ticker:aggregation:minute", Duration.ofSeconds(30), Duration.ofSeconds(20))
        JobId.TICKER_AGGREGATION_HOUR -> JobConfig(jobId, "lock:ticker:aggregation:hour", Duration.ofSeconds(60), Duration.ofSeconds(45))
        JobId.TICKER_AGGREGATION_DAY -> JobConfig(jobId, "lock:ticker:aggregation:day", Duration.ofSeconds(120), Duration.ofSeconds(90))
