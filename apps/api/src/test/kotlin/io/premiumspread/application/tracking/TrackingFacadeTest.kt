...
class TrackingFacadeTest {
    ...
    fun `AUTO 생성은 문자열 거래소를 도메인 값으로 변환하고 Result에는 문자열만 노출한다`() {
        ...
        val result = facade.recordFromMarket(openAuto())
    ...
    fun `premium snapshot 불변식 위반은 INVALID_TRACKING으로 숨기지 않는다`() {
        ...
        assertThatThrownBy { facade.recordFromMarket(openAuto()) }
    ...
    fun `목록은 Details Result로 감싼다`() {
        every { trackingService.findAllActiveByMemberId(1L) } returns listOf(TrackingFixtures.openPosition(id = 1L))

        val result = facade.findAllActiveByMemberId(TrackingCriteria.FindAllActive(1L))
    ...
    fun `summary는 목록 조회 없이 count 결과만 사용한다`() {
        every { trackingService.countActiveByMemberId(7L) } returns 2L
        every { trackingService.countArchivedByMemberId(7L) } returns 3L

        val result = facade.getSummary(TrackingCriteria.Summary(7L))

        assertThat(result.totalTrackings).isEqualTo(5)
        verify(exactly = 0) { trackingService.findAllActiveByMemberId(any()) }
    }
    ...
    private fun openAuto(koreaExchange: String = "UPBIT") = TrackingCriteria.RecordFromMarket(
        memberId = 1L,
        symbol = "BTC",
    ...
    private fun openManual() = TrackingCriteria.Record(
        memberId = 1L,
        symbol = "BTC",
        koreaExchange = "UPBIT",
        koreaQuantity = BigDecimal("0.5"),
