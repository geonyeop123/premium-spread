...
class PremiumRepositoryImplTest {
    ...
    inner class FindLatestSnapshotBySymbol {
        ...
        fun `캐시 hit이면 캐시값으로 스냅샷을 반환한다`() {
            ...
            val result = repository.findLatestSnapshotByPair(MarketPair.default(Symbol("BTC")))
        ...
        @Test
        fun `명시적 MarketPair 조회는 symbol 캐시를 사용하지 않고 같은 거래소 pair만 DB에서 조회한다`() {
            val pair = MarketPair(Symbol("BTC"), Exchange.UPBIT, Exchange.BINANCE)
            every { premiumCacheReader.get("BTC") } returns mockk(relaxed = true)
            every {
    ...
    inner class FindAggregation {
        ...
        fun `캐시와 DB의 동일 bucket은 DB를 정본으로 병합한다`() {
            ...
            every { premiumAggregationQueryRepository.findByInterval("BTC", "1h", from, to) } returns listOf(
        ...
        fun `부분 캐시는 DB의 전체 기간과 병합해 과거 bucket을 누락하지 않는다`() {
            ...
            every { premiumAggregationCacheReader.findByInterval("BTC", "1h", from, to) } returns listOf(
