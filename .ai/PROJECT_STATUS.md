...
| #44 | 페어 모델 프론트엔드 반영 (AUTO/MANUAL 폼 분리 + PnL 카드 KRW 손익 표시 포함) | 예정 |
...
- **`PositionPnl.isProfit()` 시맨틱 변경 (이슈 #43)** — `premiumDiff < 0` 기준에서 `totalPnlKrw > 0` 기준으로 변경되었다. 같은 입력 데이터에서 결과 부호가 달라질 수 있는 케이스가 존재한다 (회귀 테스트로 부호 불일치 케이스 단언). 프론트엔드는 이슈 #41 시점부터 PnL 정확 표시가 불가한 상태였으므로 사용자 영향은 없으며, 이슈 #44에서 신규 필드와 함께 동기화된다.
...
- **프론트엔드 포지션 오픈/PnL 흐름 미동기화** — `apps/web/src/components/OpenPositionForm.tsx`는 #41 시점부터 단일 거래소 필드를 전송해 오픈 호출이 실패한다. 이슈 #43에서 추가된 PnL 신규 필드(`koreaPnl`, `foreignPnlKrw`, `totalPnlKrw`, `koreaCurrentValue`, `totalPnlPercent`)도 표시되지 않는다. AUTO/MANUAL 폼 분리 + PnL 카드 확장을 포함해 이슈 #44에서 해소 예정.
