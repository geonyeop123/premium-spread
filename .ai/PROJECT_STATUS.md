...
| Module | Status | Notes |
|--------|--------|-------|
| apps/api | Active | MVP 1 백엔드 완료, JWT Stateless 인증, DB 쿼리 최적화 (N+1·인덱스·currency 컬럼), 회원 알림 구독 CRUD (이슈 #27), Position 도메인 한국/해외 페어 모델로 재구조화 (이슈 #41), Position 오픈 AUTO/MANUAL 엔드포인트 분기 (이슈 #42) |
...
| #44 | 페어 모델 프론트엔드 반영 (AUTO/MANUAL 폼 분리 포함) | 예정 |
...
- **`GET /api/v1/positions/{id}/pnl` 정확성 미보장** — 이슈 #41/#42 머지 시점에서 `PremiumService.findLatestBySymbol`은 페어 정보를 모르므로 PnL 결과가 부정확할 수 있다 (dev only). 이슈 #43에서 페어 인지 쿼리로 교체 예정.
- **PremiumSnapshot 거래소 매칭 미지원** — AUTO 엔드포인트가 사용하는 `PremiumSnapshot`은 symbol 기반이라 요청의 `koreaExchange`/`foreignExchange`와 거래소 일치 여부를 검증하지 못한다 (이슈 #42). premium 도메인이 다중 거래소 지원으로 확장될 때 해소 (별도 후속 작업).
- **프론트엔드 포지션 오픈 흐름 미동기화** — `apps/web/src/components/OpenPositionForm.tsx`는 #41 시점부터 단일 거래소 필드를 전송해 오픈 호출이 실패한다. 이슈 #42 머지 후에도 그대로이며 AUTO/MANUAL 폼 분리를 포함해 이슈 #44에서 해소 예정.
