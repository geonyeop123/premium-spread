...
class ExchangeRateServiceTest {
    ...
    inner class FindLatestSnapshot {
        ...
        fun `스냅샷은 명시된 환율 source를 보존한다`() {
            ...
            assertThat(snapshot.source).isEqualTo(Exchange.BINANCE)
        }

        @Test
        fun `기존 호출은 FX PROVIDER를 기본 source로 사용한다`() {
            val snapshot = ExchangeRateSnapshot(
                baseCurrency = "USD",
                quoteCurrency = "KRW",
                rate = BigDecimal("1432.6"),
            ...
            assertThat(snapshot.source).isEqualTo(Exchange.FX_PROVIDER)
        }
