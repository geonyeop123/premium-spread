...
class TradePreparationInvalidationTest {
    ...
    fun `시간이 아무리 지나도 무효화 사건 없이는 WATCHING 상태가 그대로다 — 시계 기반 무효화 경로가 없다`() {
        ...
        val outcome = plan.evaluateCondition(currentPremiumRate = BigDecimal("0.00"), observedAt = farFuture)
    ...
    fun `ARMED 상태에서는 evaluateCondition을 다시 호출할 수 없다 — 무기한이며 시계로 전이하지 않는다`() {
        ...
        assertThatThrownBy {
            plan.evaluateCondition(currentPremiumRate = BigDecimal("999.00"), observedAt = observedAt.plusSeconds(1))
        }.isInstanceOf(InvalidTradePreparationException::class.java)
        assertThat(plan.status).isEqualTo(TradePreparationStatus.ARMED)
    }

    @Test
    fun `INVALIDATED는 종점이다 — 다른 트리거로 재무효화를 시도해도 상태·사유가 바뀌지 않는다`() {
        val plan = watchingPlan()
    ...
    @Test
    fun `프리미엄이 희망값과 정확히 같으면 조건 충족으로 판정해 ARMED로 전이한다 — 도달이 충족이다`() {
        val plan = watchingPlan()
    ...
    fun `INVALIDATED 계획은 조건이 충족돼도 ARMED로 되돌아가지 않는다`() {
        ...
        assertThatThrownBy {
            plan.evaluateCondition(currentPremiumRate = BigDecimal("100.00"), observedAt = observedAt.plusSeconds(1))
        }.isInstanceOf(InvalidTradePreparationException::class.java)
        assertThat(plan.status).isEqualTo(TradePreparationStatus.INVALIDATED)
    }

    private fun watchingPlan(): TradePreparation {
