...
@Repository
class JpaPositionRepositoryAdapter(private val positionRepository: SpringDataPositionRepository) : PositionRepository {

    override fun save(position: Position): Position = positionRepository.save(position)

    override fun findById(id: Long): Position? = positionRepository.findByIdAndDeletedAtIsNull(id)

    override fun findAllByStatus(status: PositionStatus): List<Position> = positionRepository.findAllByStatus(status)

    override fun findAllByMemberIdAndStatus(memberId: Long, status: PositionStatus): List<Position> =
        positionRepository.findAllByMemberIdAndStatus(
            memberId,
            status,
        )
