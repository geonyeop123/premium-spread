...
data class WebSocketStreamProperties(
    ...
    @field:Valid val binance: Stream = Stream(
        URI.create("wss://fstream.binance.com/public/ws/btcusdt@bookTicker"), "BTC", "USDT",
    ),
    @field:Valid val bithumb: Stream = Stream(
        URI.create("wss://pubwss.bithumb.com/pub/ws"), "BTC", "KRW",
    ),
) {
    data class Connection(
        ...
        init {
            ...
            require(idleTimeout > firstMessageTimeout) { "market-streams.connection.idle-timeout must exceed first-message-timeout" }
    ...
    data class Stream(
        ...
        @field:NotBlank val quote: String,
    ) {
        init {
