...
class PremiumCacheService(
    ...
    override fun saveSecond(snapshot: PremiumSnapshot) = saveToSeconds(snapshot)
    ...
    fun calculateSummaryFromSeconds(symbol: String, from: Instant, to: Instant): PremiumSummary? {
        return calculateSummaryFromSeconds(MarketPair.default(Symbol(symbol)), from, to)
    }

    fun calculateSummaryFromSeconds(pair: MarketPair, from: Instant, to: Instant): PremiumSummary? {
        val data = getSecondsData(pair, from, to)
    ...
    fun calculateSummary(
        ...
        return calculateSummary(timeUnit, MarketPair.default(Symbol(symbol)), from, to)
    }
    ...
    fun aggregateSecondsData(symbol: String, from: Instant, to: Instant): PremiumAggregation? {
        return aggregateSecondsData(MarketPair.default(Symbol(symbol)), from, to)
    }

    fun aggregateSecondsData(pair: MarketPair, from: Instant, to: Instant): PremiumAggregation? {
        val data = getSecondsDataFull(pair, from, to)
        if (data.isEmpty()) return null
    ...
    fun aggregateData(
        ...
        return aggregateData(timeUnit, MarketPair.default(Symbol(symbol)), from, to)
    }
