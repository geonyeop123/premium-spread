...
`신규스크립트`는 필요하지 않으므로 `## 검사 산출물` 절은 비어 있다.
...
| AC2 | `R`·`L`·`Q` 산출이 `ECO-5` §2 관계식과 일치한다. 경계값(캡 직전·직후, 잔고 0)을 포함한다 | ECO-5 §2, design.md §3 | `상위` `ECO-5` | T1 | `신규테스트` `io.premiumspread.domain.tradeprep.TradePreparationSizingTest` | `./gradlew test --tests '*TradePreparationSizing*' --offline --no-daemon` | exit 0 |
...
| AC9 | 거래 준비 endpoint가 인증을 요구한다. `PublicEndpointPolicy`에 추가되지 않았다 | `.ai/rules/http.md` 인증 경계 | `리뷰` Phase 0 `AC9` 동형 | T1 | `신규테스트` `io.premiumspread.interfaces.api.tradeprep.TradePreparationAuthContractTest` | `./gradlew :apps:api:integrationTest --tests '*TradePreparationAuth*' --offline --no-daemon` | exit 0 |
| AC10 | owner가 응답을 보고 물량·레버리지·캡 판정이 자기 잔고와 맞는지 확인한다 | design.md §0 "중간 성취" | `추론` | T4 | 사람 확인 | 아래 `## 사람 확인` 표 | 앵커 기록됨 |

**기준 개수**: 10개

**분할하지 않는 이유**: 10개가 D1~D8 결정 여덟 개를 덮는다. AC1~AC7이 각각 다른 결정을 검증하고
서로 다른 검증 수단을 갖는다. 분할하면 durable 계획(D5)과 그 무효화(D4)·무장 전이(D7)가 다른 단위로
갈라져 상태 기계가 두 계약서에 걸친다 — 그게 더 나쁘다. AC8·AC9는 기존 gate 재사용이고 AC10은 사람 확인
하나다.

**티어 강등 사유** *(T1이 아닌 항목만)*

- AC10 → T4: owner가 자기 실계정 잔고와 대조하는 판정이며 기계가 대신할 수 없다.
  declared 어댑터 단계에서는 입력값을 owner가 준 것이므로 자기 참조가 되고, 실계정 대조는
  `ACT-2` 이후에만 가능하다. Gate 1에서 사람이 함께 승인한다.
...
DoD VERDICT: private-live-autotrader-trade-preparation @ <commit SHA>
  수용기준 표:     10개  (T1 9 · T2 0 · T3 0 · T4 1)
  T1/T2 자동:      9개 중 <p>개 PASS
  T3 기록 제출:    0개
  T4 사람 확인:    1개 중 <r>건 완료, <1-r>건 대기
  변경 요청:       <k>건
  =>
```
