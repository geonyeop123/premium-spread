...
아니라 응답의 `time_last_update_unix` — **제공자가 그 환율을 갱신한 시각**이다. 무료 플랜은 일 1회
갱신이라 그날 내내 같은 값이 온다. 실측: 07:01 과 16:14 두 번 조회했고 둘 다
`1368.4884 / observed_at 00:00:01Z` 였다(`created_at` 만 갱신됐다). 수집 기제는 정상 동작한다.
...
**선택지:**
- (a) intraday 갱신하는 유료 플랜·다른 제공자로 교체한다. 가장 직접적이지만 비용·의존성이 생긴다
- (b) 거래소에서 환율을 유도한다 — 빗썸 KRW 마켓과 바이낸스 USDT 마켓의 같은 심볼 가격비가
  실효 환율이다. 별도 제공자 없이 실시간이 되지만 USDT-USD 괴리가 섞인다
- (c) `observedAt` 나이를 응답에 드러내고 임계 초과 시 진입을 막는다. 정확도를 올리지 않고
  **거짓 정밀도를 막는** 방향이다. `D3` 의 `STALE` 라벨링과 같은 태도다
- **판단이 필요하다.** 실주문 단위 착수 전에는 정해야 한다.
