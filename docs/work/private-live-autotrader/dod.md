...
| AC3 | spec이 상태축 현재값을 중복 기록하지 않고, runtime control state와 `progress.md`의 정본 경계가 명시된다. | Review C major(M5) 상태 SSOT | T1 | 아래 `AC3 command` | exit 0, `state ssot ok` |
...
grep -q 'runtime durable control state가 정본이며' .ai/planning/private-live-autotrader/progress.md &&
...
트리거마다 해제 조건이 있고 fallback lifecycle이 정의됐는지 검사한다.

```bash
python3 - <<'CHECK'
import sys, pathlib
d = pathlib.Path('docs/work/private-live-autotrader/design.md').read_text(encoding='utf-8')
block = d.split('| 권한 회수 트리거 |')[1].split('\n\n')[0]
rows = [l for l in block.splitlines() if l.startswith('|') and not l.startswith('|---')]
bad = []
for l in rows:
    c = [x.strip() for x in l.strip('|').split('|')]
    if len(c) < 6 or not c[5]:
        bad.append(c[0][:25])
n = ' '.join(d.split())
if 'FALLBACK_CLOSED' not in n:
    bad.append('no-fallback-lifecycle')
if 'fallback 트리거는 `FALLBACK_CLOSED`에서만 해제된다' not in n:
    bad.append('no-release-binding')
print(f'rows={len(rows)} violations={bad}')
sys.exit(1 if bad else 0)
CHECK
```
...
if 'dispatch barrier를 원자적으로 세워' not in n:
    fail.append('no-dispatch-barrier')
if 'durable dispatch-claim' not in n:
    fail.append('no-dispatch-claim')
if '살아 있는 sender의 응답을 무기한 기다리지 않는다' not in n:
    fail.append('sender-liveness-dependency')
...
barrier 세대와 claim의 원자적 결속, claim–전송 구간 회수 규칙을 검사한다.
...
n = ' '.join(d.split())
fail = []
if '하나의 durable 선형화 연산' not in n:
    fail.append('no-linearization')
if '세대가 바뀐 뒤에는 어떤 sender도 새 외부 호출을 시작할 수 없다' not in n:
    fail.append('no-generation-fence')
if '회수·격리해' not in n:
    fail.append('no-ownership-reclaim')
if 'runtime durable control state' not in n:
    fail.append('no-runtime-ssot')
print(f'violations={fail}')
sys.exit(1 if fail else 0)
CHECK
```
...
stale worker 격리와 gate 결정의 runtime 전파 규칙을 검사한다.
...
fail = []
if '불가역 지점' not in n:
    fail.append('no-irreversible-point')
if 'lease와 fencing token' not in n:
    fail.append('no-lease-token')
if '호출 불능으로 격리됐음' not in n:
    fail.append('no-isolation-proof')
if 'versioned effective input' not in n:
...
- 2026-07-30 26라운드: `needs-attention`, critical 0 · high 1.
...
- 2026-07-30 28라운드: `needs-attention`, critical 0 · high 1.
  - high: `FALLBACK_HANDOFF`의 주문 목록 동결이 이미 authorization을 통과해 전송 중인 intent를 정지시키지 않아, owner
    수동 정리 직후 체결될 수 있었음 → `FENCE-0`(dispatch barrier + sender acknowledgement)을 `FENCE` 정의에 추가하고
    HANDOFF가 이를 상속하도록 함, `AC28` 강화
- 2026-07-30 29라운드: `needs-attention`, critical 0 · high 1.
  - high: `FENCE-0`의 sender acknowledgement 대기가 sender crash·ack timeout 시 `FALLBACK_HANDOFF`를 다시 교착시킴 →
    분류 근거를 `SAFE-11`의 durable dispatch-claim으로 옮기고 barrier·claim 순서를 고정, timeout·재시작 후 durable
    기록만으로 분류 완료하도록 재정의. `AC28` 재강화
- 2026-07-30 30라운드: `needs-attention`, critical 0 · high 2.
  - high: dispatch-claim이 barrier에 원자적으로 결속되지 않아 barrier 이후 claim 커밋이나 `FENCE-3` 조회 후 전송이 가능 →
    claim 생성을 barrier 세대·epoch 검증과 한 durable CAS로 묶고 claim–전송 구간의 소유·회수 규칙 추가
  - high: `progress.md`를 상태 정본으로 둔 규칙이 runtime durable 전이와 충돌 → activation·latch·`FENCE`·epoch는 runtime
    control state가 정본, 나머지 축과 승인 근거는 `progress.md`가 소유, 투영 불일치 시 fail-closed로 분리. `AC3` 갱신
- 2026-07-30 31라운드: `needs-attention`, critical 0 · high 2 (둘 다 30R 수정의 후속).
  - high: claim CAS 이후 실제 호출은 별도 단계라 세대 전환·`FENCE-3` 완료 뒤 stale worker가 호출 가능 → 불가역 dispatch
...
- GREEN: `rows=10 violations=[]`, exit 0
...
### AC28 — 2026-07-30 (29R 재강화)

- RED 개념: 29라운드가 "sender acknowledgement 대기가 sender 장애 시 교착을 만든다"를 지적했다. `FENCE-0`의 분류 근거를
  `SAFE-11`의 durable dispatch-claim으로 바꾸고 ack를 보조 신호로 낮췄으며, timeout·재시작 후 durable 기록만으로 분류를
  완료하도록 했다. 검사 조건도 claim 존재와 sender 생존 비의존으로 교체했다.
- GREEN: `violations=[]`, exit 0

### AC28 — 2026-07-30 (28R 강화)

- 강화: 28라운드가 "목록 동결이 in-flight 전송을 선형화하지 않는다"를 지적해 `FENCE-0`(dispatch barrier + sender
  acknowledgement)을 `FENCE` 정의에 추가하고 검사 조건 2개를 더했다. `FALLBACK_HANDOFF`도 이 barrier를 상속한다.
