...
| AC13 | §4.3 권한 회수 트리거 표의 모든 행이 `FENCE` 필수이고, 표 밖 guard(`SAFE-3`·`SAFE-7`·`SAFE-9`)를 포함한다. | 표 밖 guard 경로가 fence 없이 권한을 회수하던 문제(codex 7R) 차단 | T1 | 아래 `AC13 command` | exit 0, `without_fence=[] missing_sources=[]` |
...
missing = [r for r in ['SAFE-6', 'LIVE-10', 'SAFE-7', 'SAFE-9', 'SAFE-3'] if f'`{r}`' not in block]
...
- 추이: 1R 8건(critical 1) → 2R 2건(critical 1) → 3~7R 각 1~2건, critical은 2라운드 이후 0이다. 3~7라운드 지적은 모두
  "권한이 회수될 때 무엇이 허용되고 어떤 fence가 걸리는가"라는 한 매듭의 다른 표면이었고, 5·6·7라운드에서 각각 복구
  권한·전이 fence·회수 트리거를 단일 정의로 접었다.
