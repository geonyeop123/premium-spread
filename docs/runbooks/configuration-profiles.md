...
`prd`는 GitHub `production` Environment 승인 후 고정 commit SHA image로만 배포한다.

| 항목 | `local` | `test` | `prd` |
|---|---|---|---|
| 용도 | 개발자 Docker infra | Testcontainers 격리 테스트 | 승인된 SHA image |
...
| 인증/외부 secret | local 전용 기본값 허용 | deterministic test value | GitHub Environment/secret manager만 사용, default secret 기동 차단 |
