...
하네스 파일(`CLAUDE.md`·`.claude/**`)을 고쳤으면 커밋 전에 게이트를 직접 한 번 돌린다.

```bash
./gradlew harnessCheck harnessCheckTest
```

`harnessCheck`(`docs/check-harness.sh`)가 보는 것은 셋이다 — 본문(펜스 밖)의 백틱 경로와 `@import`
대상이 실재하는가, agent/skill frontmatter의 `name`이 파일명·디렉터리명과 같고 전역 유일하며
`description`이 비지 않았는가, 그리고 이 스킬과 에이전트 문서가 지켜야 할 계약(⓪·⑤ 밀도·⑥ 축·⑪ 피드백
절의 존재, ⓪→① 순서, 에이전트의 `## 재호출 시`와 완료 보고 행, `SKILL.md` 없는 스킬 디렉터리)이 남아
있는가. `harnessCheckTest`는 그 검사기가 위반을 실제로 검출하는지 픽스처로 확인한다.

**게이트가 잡지 않는 것이 남아 있다.** 단계 서술이 실제 Gradle 명령과 맞는지, 표의 내용이 참인지는
기계가 판정하지 않는다. 그래서 하네스를 고쳤으면 여전히 `CLAUDE.md ## 하네스` 변경 이력에 한 행을
남긴다 — 무엇을 왜 바꿨는지는 다음 사람이 읽을 유일한 근거다.

계약 단정을 의도적으로 버릴 때는 `docs/check-harness.sh`의 해당 `require_section`·`require_order`
호출을 **같은 커밋에서** 함께 지운다. 각 단정의 실패 메시지가 그 제거 조건을 함께 출력한다.
